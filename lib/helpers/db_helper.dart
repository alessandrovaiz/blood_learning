...
class DatabaseHelper {
  ...
  _onCreate(Database db, int version) async {
    ...
    await db.execute(
        'CREATE TABLE QUESTION (ID INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, QST_DESCRIPTION TEXT NOT NULL, QST_ISANSWERED integer DEFAULT 0, QST_ISOK integer DEFAULT 0, MODULEID integer NOT NULL, FOREIGN KEY(MODULEID) REFERENCES MODULE(ID));');
    await db.execute(
  ...
  Future<Module> save(Module module) async {
    ...
    /* await dbClient.transaction((txn) async {
      var query =
          "INSERT INTO $moduleTable ($moduleName,$isCompleted,$questions,$iconImagePath,$imagePath) VALUES ('" +
              module.moduleName +
              "," +
              module.isCompleted +
              "," +
  ...
  void resetModule({int id}) async{
    ...
    await dbClient.rawQuery('UPDATE QUESTION  SET QST_ISANSWERED = 0 WHERE MODULEID  IN (SELECT a.ID FROM MODULE a WHERE a.ID = MODULEID)');
  }
